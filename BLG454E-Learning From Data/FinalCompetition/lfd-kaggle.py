...
def weightedNeighbourSingularPredict(train_t0, train_t1, test_t0,neighbourCount):
    ...
    for i in range(train_t0_single.shape[0]):
        ...
        nbrs = NearestNeighbors(n_neighbors= neighbourCount, algorithm='ball_tree').fit(train_t0_single[i].reshape(-1,1))
        distances, indices = nbrs.kneighbors(test_t0_single[i].reshape(-1,1))
...
prediction_weightedNeighbourSingular = weightedNeighbourSingularPredict(train_t0_np, train_t1_np, test_t0_np, neighbourCount)
